...
import com.sarikaya.santiye.gunlugu.MainActivity
 ...
 */
class SantiyeGunluguWidgetProvider : AppWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        for (appWidgetId in appWidgetIds) {
            val views = RemoteViews(context.packageName, R.layout.widget_santiye_gunlugu)
            views.setTextViewText(R.id.widget_project_name, projectName)

            // SOL: Proje ismine tıklayınca → Ana Menü
            val homeIntent = Intent(context, MainActivity::class.java).apply {
                action = "com.sarikaya.santiye.gunlugu.ACTION_WIDGET_HOME"
            ...
            val projectIntent = Intent(context, MainActivity::class.java).apply {
                action = "com.sarikaya.santiye.gunlugu.ACTION_WIDGET_PROJECT"
            ...
            val cameraIntent = Intent(context, MainActivity::class.java).apply {
                action = "com.sarikaya.santiye.gunlugu.ACTION_WIDGET_CAMERA"
                putExtra("PROJECT_ID", projectId)
                flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
            }
            views.setOnClickPendingIntent(

 ...
 * Adobe Reader ve resmi belge görüntüleyicilerde Gelişmiş Belge Özellikleri (Title, Author, Creator, Subject, Keywords)
 * göstermek amacıyla ISO 32000-1 uyarınca PDF dosyasının sonuna artımlı güncelleme (Incremental Update) ile
 * Document Info Dictionary sözlüğü ekler.
 */
object PdfMetadataWriter {
    private const val TAG = "PdfMetadataWriter"

    /**
     * Oluşturulmuş PDF dosyasına harici kütüphane bağımlılığı olmaksızın resmi Metadata özelliklerini gömme işlemi yapar.
     */
    fun injectMetadata(
        pdfFile: File,
        title: String,
        ...
        try {
            RandomAccessFile(pdfFile, "rw").use { raf ->
                ...
                // Paragraf veya özel karakterleri ASCII/Literal dostu hale getir
                ...
                // 1. Yeni Info nesnesi (Document Information Dictionary)
                ...
                // 2. Artımlı Çapraz Referans Tablosu (XRef Table)
                ...
                // 3. Artımlı Römork (Trailer) Sözlüğü
            ...
            Log.e(TAG, "PDF Metadata gömme hatası (Dosya bütünlüğü korundu): ${e.message}", e)
            return false
        }
    }
